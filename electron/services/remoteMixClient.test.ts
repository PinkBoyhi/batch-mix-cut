...
describe("toRemoteAsset", () => {
  ...
  it("checks a real server health endpoint and saved Token", async () => {
    ...
    const server = http.createServer((request, response) => {
      if (request.url === "/health") {
        ...
        response.end(JSON.stringify({ ok: true, workspaceRoot: "/tmp/mix-work", audioPipelineVersion: 5, combinationPipelineVersion: 3 }));
        return;
      }
  ...
  it("rejects a server that cannot guarantee opening clip rotation", async () => {
    ...
    const server = http.createServer((request, response) => {
      if (request.url === "/health") {
        response.writeHead(200, { "content-type": "application/json" });
        response.end(JSON.stringify({ ok: true, workspaceRoot: "/tmp/mix-work", audioPipelineVersion: 5, combinationPipelineVersion: 2 }));
        return;
      }
      if (request.url === "/api/auth/check" && request.headers["x-mix-token"] === "test-token") {
        response.writeHead(200, { "content-type": "application/json" });
