...
服务器混剪节点只负责混剪和提供成片下载；云管家上传统一由桌面端在本机执行。0.1.61 组合管线升级到 v4，使每个段落的候选素材在有限批次中均匀轮换；0.1.60 音频管线升级到 v8，修复加入 BGM 后按输入路数自动压低人声的问题。客户端会阻止连接旧管线启动任务，因此必须同步升级服务器。更新前应等待正在运行及排队的任务完成。
