...
服务器混剪节点只负责混剪和提供成片下载；云管家上传统一由桌面端在本机执行。0.1.52 起音频管线升级到 v5，每个段落严格按视频流时长裁剪画面，并把原声补齐或裁剪到同一长度；桌面端会阻止旧服务器启动，避免生成静音、音画不同步或段落末帧卡住的成片。内置 FFmpeg 不可用时会自动使用服务器系统已安装的 FFmpeg。进入服务器上的本项目代码目录后执行：
